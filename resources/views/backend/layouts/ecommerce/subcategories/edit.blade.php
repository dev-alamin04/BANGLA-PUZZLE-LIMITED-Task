...
@section('content')
    <div class="content-wrapper">
        <x-breadcrumbs title="Edit Subcategory" subtitle="Update subcategory details." :breadcrumbs="[['text' => 'Subcategories', 'url' => route('subcategories.index')]]" />
                    ...
                    <form method="POST" action="{{ route('subcategories.update', $subcategory) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="description" name="description" rows="4"
                                class="form-control @error('description') is-invalid @enderror" placeholder="Optional description">{{ old('description', $subcategory->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        </div>
